...
model = torch.jit.load("models/best.torchscript", map_location="cuda:0")
...
def sliding_inference(image):
    ...
    num_horizontal_windows = (image_width) // INPUT_SIZE + \
        1 if image_width != INPUT_SIZE else 1
    num_vertical_windows = (image_height) // INPUT_SIZE + \
        1 if image_height != INPUT_SIZE else 1
    ...
    if num_horizontal_windows == 1:
        ...
        optim_overlap_width = (INPUT_SIZE-w_free_len) / \
...
def resize(image):
    ...
    original_image_height, original_image_width = image.shape[-2:]
    h_scale = original_image_height / INPUT_SIZE

    new_height = int(original_image_height / h_scale)
    new_width = int(original_image_width / h_scale)
    ...
    return image.unsqueeze(0), h_scale
...
def inference_data(original_image):
    # for _ in range(100):
    # image, crop_coord = delete_background(original_image)
    ...
    # predictions = scaling_predictions(predictions, scale)
    print(time.time()-t1)
    ...
    for prediction in predictions:
        ...
        color, class_name = classes.get(class_id, ((255, 255, 255), 'Неизвестный класс'))
        

        center_x = x1 + (x2-x1)/2
        center_y = y1 + (y2-y1)/2
        # cv2.putText(original_image, f"{class_name}, {confidence:.2f}", (x1, y1 - 10), cv2.FONT_HERSHEY_COMPLEX, 0.4, color, 1)
        cv2.circle(original_image, (int(center_x), int(center_y)), radius=7, color=color, thickness=-1)
        cv2.rectangle(original_image, (x1, y1), (x2, y2), color, 3)
        cv2.putText(original_image, f"{class_id}", (int(center_x), int(center_y)-10), cv2.FONT_HERSHEY_COMPLEX, 0.9, color, 1)
        # cv2.imwrite('img.png', original_image)
        # classes_name.append(f'\n{class_id} - означающий «{class_name}» с координатами по X: {center_x} и по Y: {center_y},')
        classes_name.append({'x': center_x, 'y': center_y, 'class_id': class_id, 'class_name': class_name})
