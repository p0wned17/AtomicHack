...
def inference_f(IMAGE_PATH, visualize_filename='visualize.jpg'):
    ...
    for prediction in predictions:
        ...
        center_x = x1 + (x2-x1)/2
        center_y = y1 + (y2-y1)/2
        defects.append({"defect": class_name, "x": center_x, "y": center_y})
        cv2.putText(original_image, f"{class_name}, {confidence:.2f}",
                    (x1, y1 - 10), cv2.FONT_HERSHEY_COMPLEX, 0.4, color, 1)
        cv2.circle(original_image, (int(center_x), int(center_y)),
                   radius=7, color=color, thickness=-1)
        cv2.rectangle(original_image, (x1, y1), (x2, y2), color, 3)

    cv2.imwrite(visualize_filename, original_image)
    return defects


if __name__ == "__main__":
    inference_f(IMAGE_PATH)
